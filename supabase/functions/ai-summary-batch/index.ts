// Supabase Edge Function: submits/polls Gemini Batch API jobs for
// ai_summary generation — kept in sync by hand with
// src/lib/ingest/aiSummaryBatch.ts (Deno can't import the Next.js app's TS
// directly), same reasoning as the ingest function.
//
// Batch mode is 50% cheaper than the old synchronous per-article Gemini
// calls (removed from the ingest function — see supabase/functions/ingest),
// but jobs are asynchronous: submit now, results ready later (verified
// empirically against the real API — ~90s for a 2-item test batch, target
// SLA up to 24h for larger ones). Each run of this function: polls whatever
// batch is currently in flight and writes back any results, then — if
// nothing is in flight — submits a new batch from the backlog of articles
// missing an ai_summary. Only one batch in flight at a time, by design.
//
// Runs every 10 minutes via pg_cron (see
// supabase/migrations/0017_ai_summary_batching.sql) — decoupled from the
// ingest function's own RSS/NewsData cadence.
//
// Deploy: paste this file's contents into the Supabase Dashboard's
// Edge Functions editor (Functions -> ai-summary-batch -> redeploy/edit),
// or `supabase functions deploy ai-summary-batch` if you have the CLI linked.
// Secrets needed (Functions -> ai-summary-batch -> Secrets): INGEST_SECRET,
// GEMINI_API_KEY (optional — this function is a no-op without it).
...
// never gets stranded. Must match
// src/lib/ingest/aiSummaryBatch.ts's flag of the same name.
const AI_SUMMARY_DISABLED = false;

const MAX_CANDIDATES_PER_BATCH = 200;

// Same reasoning and value as the old aiSummary.ts's MAX_DEK_INPUT_CHARS —
// some NewsData.io sources return their full article body as "description"
// (seen up to 15,435 chars), far more than a 2-3 sentence rewrite needs.
...
async function submitBatch(supabase: ReturnType<typeof createClient>, apiKey: string): Promise<number> {
...
async function pollAndCollectBatches(
  supabase: ReturnType<typeof createClient>,
  apiKey: string,
): Promise<{ polled: number; collected: number }> {
  ...
  let collected = 0;
  // deno-lint-ignore no-explicit-any
  for (const batch of pendingBatches as any[]) {
...
Deno.serve(async (req) => {
  ...
  if (!apiKey) {
    return new Response(JSON.stringify({ polled: 0, collected: 0, submitted: 0, note: "no GEMINI_API_KEY" }), {
  ...
  return new Response(JSON.stringify({ polled, collected, submitted }), {
    headers: { "content-type": "application/json" },
  });
});
